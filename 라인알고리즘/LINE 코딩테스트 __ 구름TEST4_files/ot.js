...
ot.CodeMirrorAdapter = (function (global) {
  ...
  CodeMirrorAdapter.prototype.setOtherCursor = function (position, color, clientId, name) { // goorm
    ...
    cursorEl.setAttribute('data-clientid', clientId);
  	var name_box = document.createElement('span');
	name_box.className = 'user_span fade_out';
	name_box.innerText = name;
	name_box.style.backgroundColor = color;
	$(cursorEl).append(name_box);
  ...
  CodeMirrorAdapter.prototype.setOtherSelection = function (selection, color, clientId, name) { // goorm
    var selectionObjects = [];
    for (var i = 0; i < selection.ranges.length; i++) {
      var range = selection.ranges[i];
      if (range.isEmpty()) {
        selectionObjects[i] = this.setOtherCursor(range.head, color, clientId, name);
      } else {
...
ot.SocketIOAdapter = (function () {
  ...
  // goorm: check filepath and project_path(exam_index or lesson_index) and quiz_index, language for multiple editor
  function SocketIOAdapter (socket, filepath, project_path, quiz_index, language) {
    this.socket = socket;
    ...
    var self = this;
    var client_left = function (clientId, _filepath) {
      if (_filepath == filepath)
        self.trigger('client_left', clientId);
    };
    var set_name = function (clientId, name, _filepath) {
      if (_filepath == filepath)
        self.trigger('set_name', clientId, name);
    };
    var ack = function (_filepath) { if (_filepath == filepath) self.trigger('ack'); };
    var operation = function (clientId, operation, selection, _filepath) {
      if (_filepath == filepath) {
        self.trigger('operation', operation);
        self.trigger('selection', clientId, selection);
      }
    };
    var selection = function (clientId, selection, _filepath) {
      if (_filepath == filepath)
        self.trigger('selection', clientId, selection);
    };
    ...
    socket
      .on('client_left', client_left)
      .on('set_name', set_name)
      .on('ack', ack)
      .on('operation', operation)
      .on('selection', function (clientId, selection, _filepath) {
        if (_filepath == filepath)
          self.trigger('selection', clientId, selection);
      })
    ...
    this.removeListener = function() {
      self.socket
        .removeListener('client_left', client_left)
        .removeListener('set_name', set_name)
        .removeListener('ack', ack)
        .removeListener('operation', operation)
        .removeListener('selection', selection)
        .removeListener('reconnect', reconnect);
    };
  }
// goormstart
  SocketIOAdapter.prototype.sendOperation = function (revision, operation, selection) {
    var time = new Date();
	var formatted = time.getFullYear() + '-' + ('0' + (time.getMonth() + 1)).slice(-2) + '-' + ('0' + time.getDate()).slice(-2) + ' ' + ('0' + time.getHours()).slice(-2) + ':' + ('0' + time.getMinutes()).slice(-2) + ':' + ('0' + time.getSeconds()).slice(-2) + ' ' + ('000' + time.getMilliseconds()).slice(-3);
	var query = {
		user: user_id,
		name: user_name,
		time: formatted,
    ...
    this.socket.emit('operation', revision, operation, selection, this.filepath, query);
  };

  SocketIOAdapter.prototype.sendSelection = $.debounce(100, function (selection) {
    this.socket.emit('selection', selection, this.filepath);
  });
// goormend
...
ot.EditorClient = (function () {
  ...
  function SelfMeta (selectionBefore, selectionAfter) {
    this.selectionBefore = selectionBefore;
	  // goorm
    if (!selectionAfter) {
      this.selectionAfter = selectionBefore;
	  this.selectionAfter.ranges[0].anchor = 0;
	  this.selectionAfter.ranges[0].head = 0;
    } else {
  ...
  OtherClient.prototype.updateSelection = function (selection) { // goorm
  ...
  EditorClient.prototype.applyOperation = function (operation) {
	this.editorAdapter.cm.grm_force_composition_end(); // goorm : Force composition end - segeon, kw
